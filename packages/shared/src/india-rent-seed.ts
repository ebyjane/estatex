...
import { pickThumbUrlsForRentRow } from './india-listing-thumbnails';

export const INDIA_RENT_TOTAL_DEFAULT = 5000;
...
interface CityRentProfile {
  ...
  lng: number;
  localities: string[];
  /** min/max monthly rent by BHK (INR) */
  rentByBhk: Record<BHK, [number, number]>;
}
...
function rentInRange(min: number, max: number): number {
  const v = rand(min, max);
  return Math.round(v / 500) * 500;
}

/** Extrapolate 4BHK band from 3BHK */
function fourBhkFromThree(
  t: [number, number],
): [number, number] {
  return [Math.round(t[0] * 1.2), Math.round(t[1] * 1.35)];
}

const CITIES: CityRentProfile[] = [
  {
    name: 'Bangalore',
    state: 'Karnataka',
    tier: 1,
...
function pickCityByTierWeight(): CityRentProfile {
  const r = Math.random();
  let pool: CityRentProfile[];
  if (r < 0.48) pool = CITIES.filter((c) => c.tier === 1);
  else if (r < 0.9) pool = CITIES.filter((c) => c.tier === 2);
  else pool = CITIES.filter((c) => c.tier === 3);
  return pick(pool);
}

function weightedBhk(): BHK {
  const r = Math.random();
  if (r < 0.22) return 1;
  if (r < 0.52) return 2;
  if (r < 0.82) return 3;
...
function buildOneRow(index: number): IndiaRentSeedRow {
  ...
  const furnishing = pick(['Semi-furnished', 'Fully furnished', 'Unfurnished'] as const);
  const bathrooms = Math.min(randInt(1, 3), bhk + 1);
  const areaSqft = randInt(400, 2000);
  const propertyType = pick(['apartment', 'apartment', 'apartment', 'house'] as const);
  ...
  const aiCategory: 'UNDERVALUED' | 'FAIR' | 'GOOD' =
    aiValueScore >= 72 ? 'UNDERVALUED' : aiValueScore >= 62 ? 'GOOD' : 'FAIR';
  const cagr5y = rand(6, 12);
  const rentalEstimate = Math.round(monthlyRent * rand(0.97, 1.03));

  const nImg = randInt(2, 3);
  const imageUrls = pickThumbUrlsForRentRow(index, nImg);
...
export function buildIndiaRentRows(count: number = INDIA_RENT_TOTAL_DEFAULT): IndiaRentSeedRow[] {
  const out: IndiaRentSeedRow[] = [];
  for (let i = 0; i < count; i++) out.push(buildOneRow(i));
  return out;
}

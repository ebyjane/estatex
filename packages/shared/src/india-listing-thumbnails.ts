 ...
 */
export const INDIA_LISTING_THUMB_POOL: readonly string[] = [
...
function poolPathSet(pool: readonly string[]): Set<string> {
...
const POOL_PATHS = poolPathSet(INDIA_LISTING_THUMB_POOL);
 ...
 */
export function pickListingThumbForProperty(propertyId: string, variant = 0): string {
  const pool = INDIA_LISTING_THUMB_POOL;
  if (!pool.length) return '';
  const idx = hash32(`${propertyId}:${variant}`) % pool.length;
  return pool[idx]!;
}

export function pickThumbUrlsForRentRow(rowIndex: number, count: number): string[] {
  const pool = INDIA_LISTING_THUMB_POOL;
 ...
 */
export function resolveListingThumbnailUrl(propertyId: string, url: string | null | undefined, variant = 0): string {
  if (!url?.trim()) return pickListingThumbForProperty(propertyId, variant);
  const u = url.trim();
  const lower = u.toLowerCase();
  if (lower.includes('/uploads/') || lower.startsWith('/uploads')) return u;

  if (LEGACY_REMAP_PHOTO_IDS.some((id) => u.includes(id))) {
    return pickListingThumbForProperty(propertyId, variant);
  }

  let unsplashPath: string | null = null;
  try {
    const parsed = new URL(u);
    if (parsed.hostname.includes('unsplash.com')) unsplashPath = parsed.pathname.toLowerCase();
  } catch {
    return u;
  }

  if (unsplashPath) {
    if (POOL_PATHS.has(unsplashPath)) return u;
    return pickListingThumbForProperty(propertyId, variant);
  }

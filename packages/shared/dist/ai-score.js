...
function calculateAIScore(inputs) {
    const { rentalYield, growth, priceAdvantage, locationDemand, riskPenalty } = exports.WEIGHTS;
    ...
    const riskScore = clamp((inputs.legalRisk + inputs.builderRisk + inputs.marketVolatility) / 3, 0, 100);
    ...
    const valueScore = clamp(Math.round(yieldComponent +
        growthComponent +
        priceAdvantageComponent +
        demandComponent -
        riskPenaltyComponent), 0, 100);
    const category = valueScore >= 80 ? 'UNDERVALUED' :
        valueScore >= 60 ? 'FAIR' :

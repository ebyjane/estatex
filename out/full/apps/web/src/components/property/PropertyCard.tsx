...
export function PropertyCard({
  ...
  return (
    <Link
      href={`/property/${id}`}
      className={clsx(
        'group relative z-10 block cursor-pointer rounded-2xl border bg-white/5 backdrop-blur-lg p-0 overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-xl',
        featured
        ...
        <div className="absolute top-3 left-3 right-3 flex flex-wrap gap-1.5 justify-between items-start z-10">
          <div className="flex flex-wrap gap-1.5">
            {aiValueScore != null && (
              <span
                className={clsx(
                  'px-2.5 py-1 rounded-lg text-xs font-bold bg-gradient-to-r text-white shadow-lg',
                  gradient,
                )}
                title="AI Value Score — investment quality vs market"
              >
                AI {aiValueScore}
              </span>
            )}

...
export function Navbar({ showAdmin = false }: { showAdmin?: boolean }) {
  ...
  return (
    <div
      role="banner"
      className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-slate-950/90 backdrop-blur-xl"
    >
      <div className="pointer-events-auto mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <Link
          href="/"
          className="relative z-[60] text-xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent"
        >
          Investify
        </Link>
        ...
        <div className="relative z-[60] flex items-center gap-2">
          <div className="relative" ref={curRef}>
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-slate-300 hover:border-cyan-500/50 hover:text-cyan-400 transition-all"
            >
              {currency}
              <ChevronDown className={clsx('h-4 w-4 transition', currencyOpen && 'rotate-180')} />
            </button>
            {currencyOpen && (
              <div className="absolute right-0 mt-1 z-[70] w-28 rounded-lg border border-white/10 bg-slate-900 py-1 shadow-xl shadow-black/40">
                {CURRENCIES.map((c) => (
                  <button
                    key={c}
                    type="button"
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="p-2 rounded-lg border border-white/10 bg-white/5 text-slate-400 hover:text-cyan-400 hover:border-cyan-500/30 transition-all"
          ...
          {user ? (
            <div className="relative" ref={userRef}>
              <button
                ...
                className="flex max-w-[160px] items-center gap-1 rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-sm font-medium text-cyan-200 hover:bg-cyan-500/20"
              >
                <span className="truncate">👤 {displayName}</span>
                <ChevronDown className={clsx('h-4 w-4 shrink-0', userOpen && 'rotate-180')} />
              </button>
              {userOpen && (
                <div className="absolute right-0 mt-1 z-[70] min-w-[180px] rounded-lg border border-white/10 bg-slate-900 py-1 shadow-xl">
                  <Link
                    href="/dashboard"
            ...
            <Link
              href="/login"
              className="rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-4 py-2 text-sm font-medium text-white hover:opacity-90 transition-opacity"
            >
              Sign In
            </Link>
          )}

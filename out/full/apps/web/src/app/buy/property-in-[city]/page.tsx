...
export async function generateMetadata({
  ...
  const title = `Buy property in ${label} | Investify — AI value & trust scores`;
  const description = `Explore verified, AI-scored homes and apartments for sale in ${label}. Multi-currency (INR, USD, AED), rental yields, and global investor tools — data from Investify only, no scraped listings.`;
  return {
    title,
    description,
    keywords: [
...
export default function BuyInCityPage({ params }: { params: { city: string } }) {
  ...
  const jsonLd = {
    ...
    description: `Listings for sale in ${label} on Investify`,
    url: `https://investify.app/buy/property-in-${params.city}`,
  };
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <p className="mt-2 max-w-2xl text-slate-400">
        Curated sale listings with AI Value Score, Trust Score, and rental yield — built for serious

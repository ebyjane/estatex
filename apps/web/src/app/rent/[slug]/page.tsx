...
export async function generateMetadata({
  ...
  if (!p) {
    return { title: 'Rentals | EstateX.ai' };
  }
  const c = titleCase(p.city);
  const title = `${p.bhk} BHK for rent in ${c} | EstateX.ai`;
  const description = `Find ${p.bhk} BHK rental homes in ${c} with trust-verified listings and AI insights. EstateX — global investor-grade search (no scraped portal data).`;
  return {
    title,
    description,
    keywords: [`${p.bhk} BHK rent ${c}`, `${c} rental`, 'verified rental', 'EstateX'],
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <p className="mt-2 max-w-2xl text-slate-400">
        ...
        supplied by EstateX partners and synthetic demo rows — never scraped from competitor sites.
      </p>
      <div className="mt-10">
        <ListingHubClient
          city={p.city}
